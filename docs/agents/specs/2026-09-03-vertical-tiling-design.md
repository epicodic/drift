...
- **Minimap**: a column's thumbnail shows its active tile only; no multi-tile thumbnail rendering.
- **Cross-row drag**: `Strip.detachFocusedColumn` currently returns a single `WindowAdapter`, matching one-window-per-column. It needs a small fix — return every window in the focused column's stack instead of just one, so `StripStack.moveFocusedWindowToRow` can re-add all of them via `addWindow` — otherwise every tile past the first would be silently orphaned (removed from the grid, geometry never forgotten) when a stacked column crosses rows. That fix is in scope this pass since it's a correctness requirement, not a new feature. What's still deferred: `addWindow` always creates a fresh column, so each re-added tile lands in its own column in the target row rather than staying stacked together — moving a stack across rows splits it back into separate columns. That's the documented MVP fallback; preserving the stack across rows is follow-up work.
