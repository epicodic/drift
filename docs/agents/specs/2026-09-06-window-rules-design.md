...
- **`width`** and **`align`**: applied once, right after the column is placed, as a post-hoc override rather than being threaded into `Strip.addWindow`'s own width computation — this avoids adding a parameter to `Strip.addWindow`'s already-tested signature (and the many call sites/tests that assert on its exact argument list) for a case that only needs to run once, immediately after the fact. `WindowManager` resolves the width (a plain number passes through; a `"NN%"` string resolves against `win.screenWidth()`) and calls `StripManager.applyRuleOverrides(win, overrides)`, which looks up the strip stack that owns the window (set synchronously by the `addTo` call moments earlier) and delegates to two new `Strip` methods: `setFocusedColumnWidth()` (sets the column's width directly, then re-renders) and `alignFocusedColumn()` (computes the target offset via the existing `alignOffsets()` from `src/viewport/align-cycle.ts` for that column and screen, then animates the viewport to it the same way `Strip.cycleAlign` already does for the manual shortcut — jumping directly to the `left`/`center`/`right` candidate instead of stepping through Strip's 3-phase cycle). Both rely on `Grid.addColumn` always focusing the column it just created, so "the focused column" is guaranteed to be the one `win` was just placed into.
