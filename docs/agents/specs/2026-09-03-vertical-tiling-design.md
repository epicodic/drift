...
- **Cross-row drag**: `Strip.detachColumn`/`detachFocusedColumn` already operate at the column level, so a stacked column's tiles move together as a unit when dragged to another row — unchanged, no new work needed, but per-tile cross-row drag is not offered.
